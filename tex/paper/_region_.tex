...
\message{ !name(paper_all_in_one.tex) !offset(554) }

Define $\mathcal{G}^{\textrm{upper} \prime}_\Phi \subset (\mathcal{P} \times \mathcal{P})^\infty$ as sequences of densities $p(x), q(x)$ that satisfy $H \equiv \int_S (\sqrt{p(x)} - \sqrt{q(x)})^2 dx = o(1)$ as well as the following conditions:

\message{ !name(paper_all_in_one.tex) !offset(4022) }
