...
%Since these two cases lead to significant differences in their respective analyses, we require different sets of assumptions for $p(x)$ and $q(x)$ for each case.
...
%To see this, note that $\pi A$ is the result of relabeling the nodes in $A$ by $\pi$ -- $A_{uv} = [\pi A]_{\pi(u), \pi(v)}$. If $\hat{\sigma}$ is permutation-equivariant, then the clustering $\hat{\sigma}(\pi A)$ is the result of relabeling the nodes in $\hat{\sigma}(A)$ by $\pi$ -- $\hat{\sigma}(A)(v) = \hat{\sigma}(\pi A)(\pi(v))$.
